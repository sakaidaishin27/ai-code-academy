...
- 【2026-07-15 実機で全面訂正】社内正本 client-starter-kit の手順には**誤りがあった**。実バイナリ（gog 0.13.0）と公式 gogcli.sh で検証した正しい手順:
  - インストール: **`brew install openclaw/tap/gogcli`**（公式 install ページ。`brew install gog` は "No available formula" で失敗＝**コマンド名 gog / パッケージ名 gogcli**）。Homebrew自体の導入は**管理者パスワードが必要でAIが代行できない** → 教材は公式 .pkg インストーラ（GUI・ダブルクリック）で案内 → 第4章 l4-2
