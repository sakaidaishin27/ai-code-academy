...
- 【2026-07-15 実機で全面訂正】社内正本 client-starter-kit の手順には**誤りがあった**。実バイナリ（gog 0.13.0）と公式 gogcli.sh で検証した正しい手順:
  - インストール: **`brew install gogcli`**（**2026-07-15 再実機で訂正**。gogcli は homebrew-core に収録済み〔`brew info gogcli` → From: homebrew-core/Formula/g/gogcli.rb〕なので tap 不要。以前記載の `brew install openclaw/tap/gogcli` は今日の環境では "No available formula or cask with the name openclaw/tap/gogcli / This command requires the tap openclaw/tap" で**失敗する**＝教材から撤去済み）。`brew install gog` も "No available formula" で失敗＝**コマンド名 gog / パッケージ名 gogcli**。Homebrew自体の導入は**管理者パスワードが必要でAIが代行できない** → 教材は公式 .pkg インストーラ（GUI・ダブルクリック）で案内 → 第4章 l4-2
  - **コマンド細部を実機再検証（2026-07-15・gog 0.13.0）**: 第4章 l4-3/l4-4 が使う `auth add <email> --services calendar,gmail --readonly`／`auth alias set main <email>`／`-a main calendar list --today`／`-a main gmail search "is:unread" --max 5` は**全フラグ・全サブコマンドが実在**（`--services` `--readonly` `--today` `--max` `auth alias set <alias> <email>` を help で確認）。加えて送信封じの `--gmail-no-send`（agent safety）も実在。
  - **【2026-07-15 スコープ整合バグを発見・修正】第4章と第10章の権限矛盾**: 第4章 l4-3 は gog を `--readonly`（calendar,gmail のみ）で認証するが、第10章 lgas1-3「ハルカゼ雑貨店を受け取る」は同じ gog に**スプレッドシートを新規作成**させる（`gog sheets create` は実在確認済）。読み取り専用では作成が失敗し、tips は「gog not found」しか案内せず初心者が詰まる。コース2内に gog 再認証は無かった → lgas1-3 の冒頭に**書き込み権限への繋ぎ直し** `gog auth add <email> --services calendar,gmail,sheets,drive --gmail-no-send`（Sheets/Drive書き込みを付与しつつメール送信は封じたまま）を追加。教材上は「第4章では読むだけ、ここで初めて作る権限を渡す」という権限漸増の教材化として整理。なお第12章のカレンダー登録・請求書PDFは**GAS(clasp)実行時のApps Script認可**を使うため gog スコープには依存しない（この矛盾は lgas1-3 のシート作成のみ）。
  - **OAuthクライアントの自作が必須**（公式 quickstart: "gog talks to Google APIs as you, using your own Cloud project"）。Google Cloudで ①projectcreate ②apis/library で Gmail/Calendar API を有効化 ③auth/branding で同意画面（External）④auth/clients で **Desktop app** クライアント作成＋JSONダウンロード ⑤**auth/audience で「アプリを公開」**（Testing のままだと**リフレッシュトークンが7日で失効**＝毎週再認証。公式明記）→ 第4章 l4-3
  - 認証: `gog auth credentials <JSON>` で鍵を登録 → `gog auth add <メール> --services calendar,gmail --readonly`（**--services と --readonly は実在。読み取り専用スコープに絞れる**＝「メール送信は人間の役目」を仕組みで担保）→ `gog auth alias set main <メール>`
  - 動作確認: `gog -a main calendar list --today`（**--today は実在**）／**`gog -a main gmail search "is:unread" --max 5`**（**旧記載の `gmail list --unread --limit 5` は誤り**。`list` は `search` の別名でクエリ引数を取る。`--unread`/`--limit` というフラグは存在しない）→ 第4章 l4-3
- ブリーフィングが動く前提: gog認証済み（読み取り専用スコープ）＋alias main＋CLAUDE.md に朝ブリーフィング定義（`gmail search "is:unread" --max 5`）→ 第4章
...
### GitHub認証（第8章・2026-07-15 実機検証）
- **`gh auth login --web --git-protocol https --hostname github.com --skip-ssh-key` は非対話環境（Claude CodeのBash）でも動く**。実機で確認: 8桁のワンタイムコード（例 `4062-5945`）と `https://github.com/login/device` を出力し、ブラウザでの承認を待ってポーリングする → **AIがコードを出し、学習者はブラウザで入れるだけ＝ターミナル不要**
- ただし承認まで終了しないので、**AIはバックグラウンドで実行**する必要がある（前面だと固まる）→ 第8章 l8-4 のプロンプトに明記
- `gh auth login` を素の対話モードで実行すると矢印キー選択のTUIになり、AIからは操作できない（フラグで回避）

### 関門（verify）の設計原則（2026-07-15 全面見直し）
- **穴の実例**: `l16-4`（免許皆伝）は check1 `contains github.io` と check2 `keywordsAny ['http','URL','github.io']` が**同時に成立**し、`passRule {min:2}` によりURL1行で修了できた。`l13-4`（L3修了）は「見出し1つ＋3行」で通り、第2章のCLAUDE.mdを貼るだけで通過できた
- **原則1**: 修了証がかかる関門は `passRule: 'all'` にし、チェックどうしが**同じ入力で二重に成立しない**よう語彙を分離する
- **原則2**: `keywordsAny` は**大文字小文字を無視する**（verify.js）。判定語に `Deny` を入れると、設定説明の `deny` に誤ヒットする（実例あり）
- **原則3**: `minChars` を必ず1本入れ、**`passRule` は `'all'` にする**。`{min: チェック数-1}` にすると**キーワードが全部当たったときに minChars が素通り**する（実例: `l8-5` が「リポジトリにコミットしてpushしました。privateです。」の31文字で通った）。minChars を「飛ばせるチェック」にしてはいけない
- **原則4**: 同型バグは横展開して潰す。`contains 'github.io'` と `keywordsAny ['http','URL']` の二重成立バグは `l16-4`（免許皆伝）と `l9-4`（第9章 Pages公開）の**両方**にあった
- **検証は `node tools/gate-attack-test.mjs`**（リポジトリに恒久化済み・全20関門に「ズル入力＝落ちるべき／正直な入力＝通るべき」の2本を流す）。**関門を触ったら必ず回す**。関門を足したらケースも足す（未テストの関門は警告が出る）
