...
### 会話・セッション操作（出典: code.claude.com/docs・2026-07-12確認）
- **/clear ＝ 空のコンテキストで新しい会話を始める**公式コマンド（別名 /new・/reset）。前の会話は消えず **/resume で復帰可能**。（commands.md）→ 第2章ほか全編の「新しい会話」の正式手段
- **settings.json（permissions・hooks）はファイル変更を監視し、実行中のセッションに再起動なしで反映される**（公式明記）。（settings.md）→ 第13章 避難訓練は会話を開き直さずそのまま実施できる
- CLAUDE.md と auto memory は「**every conversation の開始時に読み込まれる**」（memory.md）。**/clear 後の再読込は公式に明記なし**（/compact には明記あり）→ 第2章の初対面テストは結果が目で見える自己検証型にし、札が付かない場合のフォールバック（開店の1行で開き直し）を「詰まったら」に用意
- 新規カスタムコマンドの同一セッション内での認識タイミングは**公式に明記なし** → 教材は「/clear → ダメならターミナルを開き直す」の順で案内（第7章）
- /exit 等の終了コマンドは公式ドキュメントに記載なし → 教材では教えない
- **教材の設計方針（2026-07-12・実受講者のフィードバックで確立）**: 学習者がターミナルを直接操作するのは「①初回インストール（コピペ1回）②起動（開店の1行を貼る）」のみ。会話の切替は /clear。シェルコマンドの実行は全てAI＝学習者は許可・ブラウザ操作・日本語だけ。プロンプト内の生コマンドは主語（あなた=AIが実行）を明示する
