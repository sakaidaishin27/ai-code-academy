...
- 前提: **Claude Code デスクトップアプリ（Mac）**。学習者はターミナルを触らない——操作は「アプリのボタン・ブラウザ・日本語」だけ。コマンド実行はすべてAIに委譲
- 卒業時の状態: **GitHubとは何か・APIとは何か・MCPとは何かを理解し、実際に繋いだことがある**
- 図解ファースト（全レッスンにSVG図解／ターミナル再現モック等。動画は使わない——「詰まったら（症状→対処）」の先回りポイントで実演を代替）
...
| c2〔L2 実践〕仕事を任せて成果物を出す | 第5章 CSV / 第6章 リサーチ→HTML / 第7章 コマンド部品化 / 第8章 GitHubデビュー / 第9章 Pagesで公開 / 第10章 GAS入門(onEdit) / 第11章 定時レポートと見張り番 / 第12章 請求書PDFとカレンダー登録 | 成果物を作り、自分のGoogleの中で自動化を動かせる |
| c3〔L3 中級→上級〕業務フロー全体を設計 | 第13章 権限とHooks / 第14章 API / 第15章 MCP / 第16章 第二の脳 | 仕組みを設計できる |
| c4〔L4 上級〕設計し、作って、世に出す | 第17章 上級者の二つの姿勢 / 第18章 卒業制作（動くサイトを作る）/ 第19章 世に出す（Pages公開） | 免許皆伝（卒業制作を世界に公開） |
